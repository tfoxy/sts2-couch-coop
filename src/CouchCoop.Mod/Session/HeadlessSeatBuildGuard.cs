...
internal static class HeadlessSeatBuildGuard
{
    ...
    public static void EnforceOrExit()
    {
        ...
        catch (Exception exception)
        {
            ...
            Console.Error.WriteLine($"[couch-coop] seat build check skipped: {exception.GetType().Name}: {exception.Message}");
            return;
        }

        if (detail is null) return;

        CouchCoopLog.Error($"[couch-coop] seat refused: mod build does not match the host. {detail}");
        Console.Error.WriteLine($"[couch-coop] seat refused: mod build does not match the host. {detail}");
        ...
        catch (Exception exception)
        {
            ...
            Console.Error.WriteLine($"[couch-coop] seat build mismatch report failed: {exception.GetType().Name}: {exception.Message}");
        }
