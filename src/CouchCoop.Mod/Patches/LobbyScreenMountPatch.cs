...
internal static class LobbyScreenMountPatch
{
    ...
    internal static bool Apply()
    {
        lock (_sync)
        {
            ...
            CouchCoopLog.Stderr(
                $"lobby screen mount patch installed targets={plan.TargetCount - plan.Pending.Count}/{plan.TargetCount}");
            if (!complete)
            {
    ...
    internal static MethodInfo? ResolveDeclaredReady(string typeName)
    {
        ...
        {
            CouchCoopLog.Stderr(
                $"LobbyScreenMountPatch: {typeName} not found.");
            return null;
        }
        ...
        {
            CouchCoopLog.Stderr(
                $"LobbyScreenMountPatch: {typeName}._Ready not found.");
            return null;
        }
        ...
        if (target.DeclaringType != type)
        {
            ...
            CouchCoopLog.Stderr(
                $"LobbyScreenMountPatch: {typeName} does not declare _Ready "
    ...
    private static void ReadyPostfix(Node __instance)
    {
        ...
        catch (Exception exception)
        {
            ...
            CouchCoopLog.Stderr(
                $"LobbyScreenMountPatch: mount note failed: {exception.GetType().Name}: {exception.Message}");
        }
    }
}
