...
public static class LanAddressRanking
{
    ...
    public static IReadOnlyList<LanAddressCandidate> GatherFromOs(Action<string>? log = null)
    {
        ...
        {
            log?.Invoke($"[couch-coop] lan-address enumeration failed detail={exception.GetType().Name}:{exception.ErrorCode}");
            return [];
        }
        catch (PlatformNotSupportedException exception)
        {
            log?.Invoke($"[couch-coop] lan-address enumeration unsupported detail={exception.GetType().Name}");
            return [];
        }
        ...
        foreach (var networkInterface in interfaces)
        {
                ...
                or NotSupportedException)
            {
                ...
                // rest of the list.
                log?.Invoke($"[couch-coop] lan-address interface skipped detail={exception.GetType().Name}");
            }
        }
...
#pragma warning restore CA1416
    ...
    public static string? ValidateAdvertisedHostOverride(string? raw, Action<string>? log = null)
    {
        ...
        if (IPAddress.TryParse(unbracketed, out var address))
        {
            ...
            {
                log?.Invoke($"[couch-coop] advertised-host override ignored value={value} reason=wildcard-address");
                return null;
            }
        ...
        {
            log?.Invoke($"[couch-coop] advertised-host override ignored value={value} reason=malformed-ip-literal");
            return null;
        }
        ...
        log?.Invoke($"[couch-coop] advertised-host override ignored value={value} reason=not-an-address-or-hostname");
        return null;
    }
}
