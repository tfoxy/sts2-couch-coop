...
- **Static backgrounds and cache:** combat URLs are `/bg/{id}?layers={digest}&v=1`; event and room backgrounds
  use `/bg/events/{id}?v=1` and `/bg/rooms/{id}?frame={frame}&v=2`. Their codec is named by `Content-Type`. A seat
...
## Baked effect stills (shaders / particles OFF or STATIC)
...
- **What they are for, in OFF.** `Shaders: Off` / `Particles: Off` is what `QUALITY_PRESETS.minimum` writes and
  what the hard-off floor forces on a software-WebGL phone. Before these, that configuration rendered NOTHING
  for the card playable-glow (`card_ripple`) and the rarity shimmers — both gameplay cues, not decoration.
- **What they are for, in STATIC** (the product default on every device). That mode renders one frozen frame
  per surface and `shaderResources.staticSurfacePolicy` then swaps each quiet canvas for an `<img>` — a GPU→CPU
  readback plus a PNG encode, per surface, whose cost every tuned constant in that file exists to bound. These
  bakes ARE those frames, already encoded, so the three covered effects cost one decode instead. `card_ripple`
  leaves that fleet entirely, which matters most: its content key churns on `width`, so a hand used to re-encode
  through every playability tween.
- **The three dynamic modes keep the live path.** They exist to animate; a still is not a cheaper animation.
- **A covered node's live BINDING is not built.** `shaderAttributes.computeShaderAttributes` and
  `particleAttributes.nodeParticleAttributes` both early-return on `bakedStillCoversNode(node)`, so no
  `data-godot-shader-*` / `data-godot-particle-*` markers are stamped and gsw never selects the node — otherwise
  the viewer gets the still AND the canvas. Two traps around that gate:
  - **Do not make `isWebglShaderNode` false for a covered node.** It is what suppresses the node's raw
    `card_frame_sdf.exr` paint in `nodeStyles.paintsTexture` and in `canvas/paintSpec`, and its other
    suppression term reads the TIER flag, which is true in `static`. Flip it and both stages paint the bare SDF
    as a grey rectangle under the still. It keeps meaning "structurally a WebGL node".
  - **`shaderContentKey` must carry the effect mode.** The L2 memo was keyed on the tier's `shadersEnabled`,
    which does not move when the panel does; without the mode a viewer flipping Static → Dynamic is served a
    cached `null` and never gets their shaders back. The particle side needs no such term only because its gate
    sits ahead of `specsJsonCache`.
- **Three art contracts, each load-bearing.** (1) The stills are ADDITIVE: captured over opaque black and
  converted to straight alpha, so `rgb × alpha` is the contribution and they need `plus-lighter`. The glows get
  that from their own streamed `canvasBlendMode: 1`; the ripple's blend is declared by the SHADER, so
...
- **What a viewer sees change in STATIC**, and neither is a defect: the ripple's arbitrary phase (its shader
  ...
  double-applied modulate or a missing `plus-lighter`.
- A mode flip needs a FULL walk (`scheduleRender(true, "effects")` in MirrorView) — crossing between the still
  modes and the dynamic ones changes what the walk emits, and a settled screen visits no nodes.
- Tests: `frontend/src/mirror/__tests__/bakedEffects.spec.ts` (selection + the suppression/paint agreement),
  `bakedEffectMount.spec.ts` (mount + placement + where the blend comes from + no markers, driven by an emitter
  lifted verbatim out of a recorded session), the `nodeStyle baked effect stills` block in `nodeStyles.spec.ts`,
  and the `card_ripple width gate` blocks in `shaderAttributes.spec.ts` / `mirrorPureMemos.spec.ts`.
...
- With the family OFF a particle node paints nothing at all — except the two card rarity glows, which fall back
  to a committed still (see **Baked effect stills** above). In STATIC those same two glows take the still as
  well, and `nodeParticleAttributes` stamps no markers for them at all.
...
| no contact at all ⇒ killed at 35s | `COUCHCOOP_SEAT_CONTACT_TIMEOUT_SECONDS` (default `DefaultSeatContactTimeoutSeconds` = 35, same clamp band and parse rule as `COUCHCOOP_SEAT_READY_TIMEOUT_SECONDS`). **Which failure the silence is turns on host lobby membership, then on the seat's own port record** (`77b2f7e1`, `ClassifySilentSeat`): a member can only have joined through `CommandLineOverridePatch` — which runs after the isolation guard — so its saves are provably covered and it is failed as `SeatSilentAfterJoinCode` (or `SeatControlBlockedCode` if it is still serving). **A non-member is NOT thereby unmodded**: a seat joins only after the game's asset preload, so a healthy one on a slow machine is still outside the lobby at 35s. A pid-matched `browser-port-slot-N` record clears it (only our browser server writes one, after the guard) → `SeatControlBlockedCode`; no probe is spent on a non-member, since a probe answer cannot clear it and costs ~2s on Windows. Only a non-member with no record of its own keeps `SeatCloudIsolationCode`, whose detail says the host *cannot confirm* CouchCoop is running and carries the same `Observed:` evidence tail as the other arms. The seat's hello writes its status-file fallback on a **deadline** too, not only on a socket error — a filtered connect always hits the hello's 2s deadline first, which used to leave that seat with no file until the live reporter's first heartbeat. Survivable for a healthy seat **because of the hello**: the only work inside the window is game boot up to mod init, not the 20-30s asset preload a handheld runs past while healthy. **Measured 2026-09-17** (dev desktop, 12 threads): first contact at 4.1s / 4.3s idle and **9.9s with every core saturated**, which is what keeps the default at 35 rather than the 20 the hello makes plausible — grade a proposal against the loaded figure, and note that no handheld has reported one. Every seat's real figure is logged once — `seat first contact slot=N afterMs=…`, **stderr only, never in godot.log** — and qa-recipes §7 makes reading it a step. **This SHRINKS the exposure window, it does not close it**: an unmodded seat runs its own cloud sync inside those 35 seconds, and its process actually lives for the deadline **plus** the 5s graceful-shutdown deadline, because it has none of our code to answer the stop. The safety net for that case is the host-side pre-spawn profile backup in the row below | `HeadlessClientManager.cs`, `HeadlessClientManager.Connections.cs` |
...
stops it on its first heartbeat. The contact deadline covers a seat running *nothing of ours at all* — and,
indistinguishably, a seat of ours that neither reached the lobby nor recorded its port — and is the only one of
the three that is a race rather than a guarantee: it is honest mitigation, not protection.
