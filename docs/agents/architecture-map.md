...
| no contact at all ⇒ killed at 20s | `COUCHCOOP_SEAT_CONTACT_TIMEOUT_SECONDS` (default `DefaultSeatContactTimeoutSeconds` = 20, same clamp band and parse rule as `COUCHCOOP_SEAT_READY_TIMEOUT_SECONDS`). Safe at 20 **because of the hello**: the only work inside the window is game boot up to mod init, not the 20-30s asset preload a handheld runs past while healthy. Every seat's real figure is logged once — `seat first contact slot=N afterMs=…` — and qa-recipes §7 makes reading it a step. **This SHRINKS the exposure window, it does not close it**: an unmodded seat runs its own cloud sync inside those 20 seconds. The actual safety net is a host-side pre-spawn profile backup, which does not exist yet | `HeadlessClientManager.cs`, `HeadlessClientManager.Connections.cs` |
