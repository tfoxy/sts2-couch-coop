...
- **The browser makes gameplay happen through REAL INPUT, not through spirectl semantic actions.** A viewer's tap becomes the same hover / press / release / key events a player at the keyboard produces, replayed at a resolved coordinate, and the game's own widget decides what that means. Do **not** reach for a spirectl semantic action (`claim-reward`, `select-card`, `buy-*`, …) to commit a player's choice: a semantic action re-implements the game's commit path, and a re-implementation drifts — see [docs/agents/architecture-map.md](docs/agents/architecture-map.md) "Real input, not semantic actions" for the case that set this rule (a refused potion reward was destroyed because the bridge retired a row the game had declined). **Any use of a semantic action needs the maintainer's explicit go-ahead for that specific case.** If you think you need one, ask — do not adopt it because it is the shorter path. Read-only spirectl surfaces (state, scene stream, screenshots, inspection) are unaffected; this rule is about causing state changes.
